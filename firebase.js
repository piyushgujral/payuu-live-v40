...
window.firebaseDB = {
  ...
  checkAdminStatus: function(userEmail, callback) {
    db.ref("admins").once("value", (snapshot) => {
      ...
      Object.keys(admins).forEach(uid => {
        if (
          admins[uid].email &&
          admins[uid].email.toLowerCase() === userEmail.toLowerCase()
        ) {
          matchedAdmin = {
            uid,
            ...admins[uid]
  ...
  toggleAdminActive: function(key, activeState) {
    return db.ref("admins/" + key).update({
      active: activeState
    });
  },
  ...
  uploadVoiceRecording: function(blob, fileName, contentType) {
    ...
    if (!blob) {
      return Promise.reject(
        new Error("No voice recording supplied.")
      );
    }
    ...
    if (blob.size > 5 * 1024 * 1024) {
      return Promise.reject(
        new Error("Voice recording exceeds the 5 MB limit.")
      );
    }
  ...
  addPendingSupport: function(data) {
    ...
    return newRef.set({
      ...
      msg: data.msg || "",
      messageSource:
        data.messageSource || "text",
      voiceUrl: data.voiceUrl || "",
      voiceMimeType:
        data.voiceMimeType || "",
      voiceDuration:
        Number(data.voiceDuration || 0),
      voiceStatus:
        data.voiceStatus || "none",
      voiceEnabled:
  ...
  removeOverlayAlert: function(key) {
    return db
      .ref("overlayQueue/" + key)
      .remove();
  },

  listenPending: function(callback) {
    db.ref("pendingSupport").on(
      "value",
      (snapshot) => {
  ...
  resetStatistics: function() {
    return Promise.all([
      db.ref("approvedSupport").remove(),
      db.ref("overlayQueue").remove()
    ]);
  }
};
...
(function installOriginalSupporterVoiceBridge() {
  ...
  // Only voice submissions are intercepted.
  // Typed-only submissions continue through the existing master script.
  ...
  // For voice submissions, upload the original audio after payment confirmation,
  // then allow the normal admin-email flow.
  ...
  document.addEventListener(
    ...
    () => {
      ...
      window.__payuuOriginalVoiceAudio =
        audio;


      setTimeout(() => {

        const originalSpeakAlertVoice =
          window.speakAlertVoice;
        ...
        window.speakAlertVoice =
          function(item, onComplete) {
            ...
            // use the normal TTS engine.

            if (
              !url ||
              item.voiceEnabled !== true ||
              item.voiceStatus ===
                "rejected"
            ) {

              if (
                typeof originalSpeakAlertVoice ===
                "function"
              ) {

                originalSpeakAlertVoice(
                  item,
                  done
                );
            ...
            try {
              ...
              if (
                siteSettings?.overlay
              ) {
                siteSettings.overlay.duration =
                  needed;
              }
